...
```{r}
load("D:/Projects/HWZ/lib/Umfrage.rda")
```
...
Aus dem Dataframe der **`r nrow(df)`** Datensaetze hat, die ersten **`r subObservations`** Datensaetze nehmen und daraus einen neuen erzeugen.
```{r}
...
set.seed(678)
titanic <-read.csv("D:/Projects/HWZ/lib/titanic.csv")
...
### Prediction
```{r fig.cap="8.2 Confusion Matrix", fig.align="center"}
predict_unseen <-predict(fit, data_test, type = 'class')
table_mat <- table(data_test$survived, predict_unseen)
table_mat
```

Table 8.2 zeigt, dass das Modell korrekt 106 Passagiere als tot vorhersagt, jedoch faelschlicherweise 15 ueberlebende als tot. In der zweiten Zeile sind die vom Modell vorhergesagten Ueberlebenden, wobei das Modell 30 Passagiere als Survivors vorhersagte, obwohl sie gestorben sind.

```{r fig.cap="8.3 Vorhersage", fig.align="center"}
...
Die Vorhersage in Fig. 8.3 zeigt, dass ich wahrscheinlich gestorben waere.
