...
```
# Einleitung
Diese Arbeit wurde in einem **RStudio-Project** mit **RMarkdown** und **Shiny** umgesetzt. Die Shiny-App ist lediglich ein Proof of Concept um mal das Zusammenspiel zu erproben. Sie besteht lediglich aus einem Schieber, mit dem die Anzahl Beobachtungen fuer einen zufaellig generierten Dataframe eingestellt werden kann und einem Button, der dann das RMarkdown rendert (knitted). Das komplette **RMarkdown** wie auch der **Code** ist auf **[Github](https://github.com/knoeflerm/HWZProject)** verfuegbar. Die Arbeit sollte somit fuer jeden **reproduzierbar** sein.
\
...
## Daten einlesen
Es gibt verschiedene Moeglichkeiten Daten zu lesen. Der Readbefehl haengt vom Dateityp der Quelle ab. Die wichtigsten sind:
...
### Subset
Subdatensaetze mit gewissen Kriterien kann man auf verschiedene Arten auslesen, z.B. mit dem Befehl **which**, oder **subset**.
```{r}
...
### Matrix
Der Data Frame wird in eine Matrix konvertiert und mit anderen Spalten- und Zeilennamen versehen. Die Befehle as.matrix() und data.matrix() konvertieren die Daten unterschiedlich. Bei as.matrix() werden alle Daten in Character umgewandelt, der Datentyp geht somit verloren. So kann z.B. der summary() Befehl keine Mediane oder Means mehr berechnen.
```{r}
...
# Tag 4 - Hypothesen
Im Weiteren werden nicht mehr alle R Code Snippets im Report ersichtlich sein (echo=FALSE). Das vollstaendige Markdown mit R-Code kann von [Github](https://github.com/knoeflerm/HWZProject) herunter geladen werden.
...
* **Analyse** erstellen. Ursache-Wirkung anhand von einem Datenmodell, Fragebogen erstellen. Entweder existieren bereits brauchbare Daten die gesammelt werden muessen, oder sie muessen erhoben werden.
* Nun kann ein **Entscheid** formuliert werden. Besagt der Test, dass die Hypothese signifikant ist, koennen Handlungsoptionen abgeleitet werden und die Wirkung der Massnahmen (theoretisch) gemessen werden.
Die **Qualitaet** des Entscheids haengt von drei Aspekten ab:
    ...
    + **Signifikanz**: Das Resultat ist nicht zufaellig und ist bei einer zweiten Analyse in etwa gleich. Auch hier, je groesser n desto signifikanter das Resultat.
    + **Relevanz**: Das Resultat der Analyse kann einen Entscheid untermauern und es koennen Prognosen gemacht werden.

\
Um etwas mit Daten spielen zu koennen, wird ein Dataframe d mit Zufallszahlen generiert.
\
...
## Tests
Tests werden verwendet um zu bestimmen, ob Stichproben (zwei beim T-Test, mehrere beim Anova-Test) sich statistisch signifikant unterscheiden.
...
```
In Fig 4.3 kann man gut sehen, dass Autos mit Handschaltung (1) tendenziell weiter kommen mit einer Gallone Treibstoff als Autos mit einem Automatikgetriebe.

# Tag 5 - Prognosen mittels Regressionsanalyse
## Regression
### Korrelation
Die **Korrelation** von -0.852 zwischen 'Miles per Gallon' und 'Cylinder' ist nicht gerade gross, fuer dieses Beispiel jedoch ausreichend. Die negative Korrelation bedeutet, dass je mehr 'Cylinder' ein Auto hat, desto weniger weit kommt es mit einer Gallone.
...
```
Fig. 5.3 zeigt einen Korrelations Scatter Plot mit etwas mehr Informationsgehalt. Die Korrelationskoeffizienten, Histogramme mit Density und die Scatterplots die auch in Fig. 5.2 dargestellt sind. Fuer rcorr kann man auch den type="pearson" verwenden was unterschiedliche Korrelationswerte ergibt. Hier muss man je nach Anwendungsfall entscheiden welchen type man verwenden sollte.
...
### Variablen Selektion
Das Ziel der Variablenselektion ist, aus einer großen Auswahl von potentiell erklärenden
Variablen in einem Datensatz diejenigen herauszufinden, mit denen ein Regressionsmodell
die Vorhersage der Zielvariablen am besten beschreibt. Die Variablenauswahl erfolgt zum einen aus inhaltlichen Gesichtspunkten und zum anderen mit Hilfe unterschiedlicher statistischer Verfahren. Bekannte Selektionsmethoden sind beispielsweise die Backward-, Forward- oder das Stepwise-Verfahren
\
...
## Text bereinigung
Zuerst muessen die Texte bereinigt werden. Alle "nichtssagenden" Woerter (Stopwords) raus wie auch andere unbenoetigte Zeichen (#, @, ., ?, u.s.w.). Auch werden die Woerter ge-clustert und ihrem Wortstamm zugeordnet (Stemming).
...
### Commonality
Eine Comparison Cloud vergleicht zwei (aehnliche) Datasets. Man kann erkennen was in unserem Fall z.B. die Staedte Zuerich und St.Gallen gemeinsam haben.
...
## Regression Tree
Regression Trees haben ein kontinuierlich abhaengiges Merkmal und muessen somit mit der Methode 'anova' berechnet werden.
\
...
In Fig. 8.2 kann man im Root-Leaf sehen, dass

* das Durchschnittsalter 30 Jahre betraegt
* eher aeltere Leute konnten sich ein Erstklassticket leisten
* von den Ueberlebenden mit Erstklassticket war das Durchschnittsalter 36
* von den gestorbenen war das Durchschnittsalter 43, was 10% aller Passagiere ausmacht
...
### Prediction
Zum Vergleich eine Prediction mit dem geschaetzten linearen Modell auf den Trainingsdaten
```{r}
