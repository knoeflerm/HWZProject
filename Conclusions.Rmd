...
# Tag 3
Ein paar Plots und Histogramme auf dem originalen Dataframe. In den Grafikbeschreibungen ist jeweils der entsprechende R Code aufgefuehrt. Da saemtliche Daten im Dataframe zufaellig erzeugt wurden, sehen die Grafiken und Kurven natuerlich auch willkuerlich und etwas chaotisch aus. Ich musste allerdings das Dollar Zeichen mit einem Punkt ersetzen (z.B. df$age => df.age), da ansonsten RMarkdown (oder LaTeX) den Dollar interpretiert.

```{r ,echo=FALSE, fig.cap="hist(df.age, prob=T) lines(density(df.age),col=\"red\")", fig.align="center"}
hist(df$age, prob=T)
lines(density(df$age),col="red")
```

```{r ,echo=FALSE, fig.cap="plot(df.age, df.income, type=\"p\")", fig.align="center"}
plot(df$age, df$income, type="p")
```

```{r ,echo=FALSE, fig.cap="plot(df.income,main=\"Normalverteilung\")", fig.align="center"}
plot(df$income,main="Normalverteilung")
```

```{r ,echo=FALSE, fig.cap="hist(df.income,main=\"Gleichverteilung\")", fig.align="center"}
hist(df$income,main="Gleichverteilung")
```

```{r ,echo=FALSE, fig.cap="hist(df.income,main=\"Binomialverteilung\"))", fig.align="center"}
hist(df$income,main="Binomialverteilung")
```

```{r ,echo=FALSE, fig.cap="hist(df.income,main=\"Normalverteilung\")", fig.align="center"}
hist(df$income,main="Normalverteilung")
```
